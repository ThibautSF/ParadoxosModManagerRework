...
//	private ObservableList<ModList> selectedListsList = FXCollections.observableArrayList();
	...
	private HBox actionButtons = new HBox(8);
	...
	private HBox actionButtons2 = new HBox(8);
...
//				}
		...
		// Buttons line 1
		window.add(actionButtons, 1, 3, 4, 1);
		actionButtons.setStyle("-fx-alignment: bottom-center;");
		actionButtons.getChildren().addAll(newList, modifyList, delList, applyList);
		modifyList.setDisable(true);
		delList.setDisable(true);
		applyList.setDisable(true);

		// Buttons line 2
		window.add(actionButtons2, 1, 4, 4, 1);
		actionButtons2.setStyle("-fx-alignment: top-center;");
		actionButtons2.getChildren().addAll(importList, exportList);
		exportList.setDisable(true);
		...
		String alertConfirmTitle = "Confirmation";
		String alertConfirmHeader = "Confirm !";
		...
		delList.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent t) {
				...
				String message = "Are you ok to delete '" + toDelete.getName() + "' ?";

				Optional<ButtonType> result = BasicDialog.showGenericConfirm(alertConfirmTitle, alertConfirmHeader,
						message, null);
		...
		applyList.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent t) {
				...
				String message = "Are you ok to apply '" + toApply.getName() + "' ?" + strInfo;
				...
				if (result.get() == ButtonType.OK) {
					try {
						if (applyModList(toApply)) {
							...
							buttons.add(buttonTypeLaunchGame);
							buttons.add(buttonTypeCancel);

							Optional<ButtonType> resultInfo = BasicDialog.showGenericConfirm("Success", null,
									"The list was successfully applied !", buttons);
